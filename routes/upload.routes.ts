...
uploadRouter.post('/upload/video', authUser, authAdmin, (req: Request, res: Response) => {
    try {
        ...
        cloudinary.uploader.upload(file.tempFilePath, { resource_type: "video", folder: "KAIDEE MONG" }, async (err: cloudinaryResult.UploadApiErrorResponse | null, result: cloudinaryResult.UploadApiResponse | undefined) => {
            if (err) throw err;
            removeTmp(file.tempFilePath);
            res.json({ public_id: result?.public_id, url: result?.secure_url });
        })
